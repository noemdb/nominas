<!-- Modal para ver detalles de descuento -->
<x-modal
    title="Detalles de Descuento"
    :max-width="'2xl'"
    ...
    @if ($discountDetails)
        <div class="space-y-6">
            ...
            <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800/50 dark:border-gray-700">
                ...
                <div class="space-y-3 pl-7">
                    <p class="flex items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->name }}</span>
                    </p>
                    <p class="flex items-start space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Descripción:</span>
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->description ?? 'N/A' }}</span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Tipo:</span>
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $discountDetails->type === 'fijo' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                            {{ ucfirst($discountDetails->type) }}
                        </span>
                    </p>
                    @if ($discountDetails->type === 'fijo')
                        <p class="flex items-center space-x-2">
                            <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Monto:</span>
                            <span class="text-sm text-gray-900 dark:text-gray-100">Bs. {{ number_format($discountDetails->amount, 2) }}</span>
                        </p>
                    @else
                        <p class="flex items-center space-x-2">
                            <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Función:</span>
                            <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->name_function ?? 'N/A' }}</span>
                        </p>
                    @endif
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Moneda:</span>
                        <span class="flex items-center">
                            <svg class="w-4 h-4 {{ $discountDetails->status_exchange ? 'text-yellow-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span class="ml-1 text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->status_exchange ? 'USD' : 'Bs.' }}</span>
                        </span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Estado:</span>
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $discountDetails->status_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
            ...
            <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800/50 dark:border-gray-700">
                ...
                <div class="space-y-3 pl-7">
                    <p class="flex items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->institution ? $discountDetails->institution->name : 'No aplica' }}</span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Área:</span>
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->area ? $discountDetails->area->name : 'No aplica' }}</span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Rol:</span>
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->rol ? $discountDetails->rol->name : 'No aplica' }}</span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Posición:</span>
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->position ? $discountDetails->position->name : 'No aplica' }}</span>
                    </p>
                    <p class="flex items-center space-x-2">
                        <span class="w-32 text-sm font-medium text-gray-500 dark:text-gray-400">Trabajador:</span>
                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $discountDetails->worker ? $discountDetails->worker->full_name : 'No aplica' }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800/50 dark:border-gray-700">
                ...
                <div class="pl-7">
                    <x-payroll-details-table :payrolls="$discountDetails->payrolls" />
                </div>
            </div>
        </div>
    @endif
